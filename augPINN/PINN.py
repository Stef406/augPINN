...
output_dir = os.path.join(os.path.dirname(path), 'output_images_500-2_PINN')
...
for i in range(iterations):
    ...
    if i % 100 == 0:
        # print(f"Iteration {i}: Loss_phy = {loss_phy.item()}, Loss_data = {loss_data.item()}, Loss train = {loss.item()}, Loss test = {loss_test.item()}")
        ...
        plt.rcParams['axes.spines.right'] = False
        plt.rcParams['axes.spines.top'] = False
        plt.scatter(t, average_pos/H_bed, color='black', label='Experiments', s=5)  # Scatter plot for the average
...
plt.rcParams['axes.spines.right'] = False
plt.rcParams['axes.spines.top'] = False   
plt.scatter(t, average_pos/H_bed, color='black', label=f'Experiments (T={T}, FI={r})', s=5)  # Scatter plot for the average
plt.fill_between(t, (average_pos - std_dev)/H_bed, (average_pos + std_dev)/H_bed, color='gray', alpha=0.3)  # Error band
plt.plot(t_plot, u_pred/H_bed, label="PINN solution", color="red")
plt.xlabel("t [s]")
plt.ylabel(r"$Z_p$ [-]")
...
plt.text(x=min_val + min_val*0.01, y=max_val - max_val*0.02, 
         s=f'$R^2$ = {r2_max:.3e}', fontsize=15, 
         verticalalignment='top', horizontalalignment='left')
...
plt.text(x=min_val + min_val*0.01, y=max_val - max_val*0.080,
         s=f'T = {T}', fontsize=15, 
         verticalalignment='top', horizontalalignment='left')
plt.text(x=min_val + min_val*0.01, y=max_val - max_val*0.11,
